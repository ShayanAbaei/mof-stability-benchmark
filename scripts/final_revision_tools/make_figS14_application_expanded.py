...
def make_better_s14():
    ...
    axA.set_xticks(x)
    axA.set_xticklabels(labels, rotation=45, ha="right")
    ...
    axB.set_xticklabels([CATEGORY_LABEL[c].replace("\n", " ") for c in CATEGORY_ORDER], rotation=45, ha="right")
    ...
    cbar.set_label("log10(entries + 1)")
    ...
    axD.set_xticks(x)
    axD.set_xticklabels([lab for lab, _, _ in tiers])
    axD.set_ylabel("Chemistry classes")
    axD.set_title("Operational triage tiers", fontweight="bold")
    axD.grid(axis="y", alpha=0.22)

    # Add a small note showing whether a true predicted shortlist CSV was found.
    note = "Predicted shortlist CSV found" if true_shortlist is not None else "Class support from coverage map; predicted shortlist CSV not found"
    axD.text(0.02, -0.26, note, transform=axD.transAxes, fontsize=5.8, ha="left", va="top", color=COL["dark"])
    add_panel_label(axD, "D", x=-0.12, y=1.04)
