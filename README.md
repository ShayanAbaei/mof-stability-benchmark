...
Code and documentation for a reproducible descriptor-sufficiency benchmark of
metal-organic framework (MOF) water, solvent-removal/activation, and thermal
stability.
...
documentation. It should not redistribute the external raw database files or
large generated prediction/output files.
...
docs/
  ...
  archive_inventory_recommendations.csv
scripts/
  01_audit_scripts/
  02_main_sweep/
  03_post_processing/
...
- generated figures/tables unless you intentionally create a small `examples/` folder.

The external raw input files should be downloaded from the official data
sources described in `data_README.md` and
`docs/DATA_PROVENANCE_AND_CITATIONS.md`.

## Required local input files

Place the following files locally in `02_raw_data/` or `raw_data/` before
...
These files are from the CoRE MOF 2024 Zenodo dataset release and should be
cited separately.
...
conda activate project15_mof_stability
```
...
Use `make_final_main_figures_STRICT_v4.py` for final main-figure regeneration.
It intentionally avoids silent fallback behavior. If a required source CSV is
missing, it stops rather than switching to a different table. This protects the
scientific identity of panels such as the Figure 4 error analysis.

The older script `make_final_cleaned_main_figures.py` should be treated as
deprecated and should not be used for final manuscript figures.

## Data provenance

Please see `data_README.md` and `docs/DATA_PROVENANCE_AND_CITATIONS.md`.

## Citation

Please cite the associated manuscript if you use this repository. Please also
cite the external database and stability-label sources listed in `data_README.md`.
